...
`clients` is the business source of truth. `websites` has one persisted configuration per client and stores template selection independently from generated content. `domains` has one current setup record per client and tracks availability, ownership (`available_not_owned`, `existing_owned_domain`, `purchase_pending`, `purchased`), purchase, zone activation, nameservers, custom domain, HTTPS, and separate Cloudflare/public Meta TXT states. `integration_runs` records safe operational outcomes for `agentrouter`, `hostinger`, `cloudflare`, and `system`.
...
`proxy.ts` normalizes the request hostname. Internal hosts, localhost, and Workers preview hosts retain normal App Router behavior. A client hostname is rewritten to `/sites/{hostname}/{optional-path}`.

`app/sites/[hostname]/[[...path]]/page.tsx` resolves the normalized hostname through `domains`, loads the associated client and persisted website, and serves `/`, `/privacy`, and `/terms`. All other paths return 404. One Worker and repository serve every client domain; unknown hostnames do not fall through to another client.
...
-> Worker custom domain -> HTTPS check
...
- `LOGO_ASSETS`: optional client-logo R2 bucket.

The default Wrangler configuration remains usable with local D1. The production environment intentionally uses placeholders for the remote D1 database and R2 bucket. Production provisioning requires creating those resources, replacing placeholders, setting Worker secrets/account inputs, applying migrations remotely, deploying the shared Worker, and attaching real domains. No secrets belong in `wrangler.jsonc` or tracked environment files.
